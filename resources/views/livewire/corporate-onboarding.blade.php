        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @foreach ($documents as $key => $document)
                <tr class="text-gray-700">
                    @if($loop->index == 0)
                        <th rowspan="19" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            To complete by client
                        </th>
                    @endif
                    <td class="border px-4 py-2" colspan="2">{{ ${$document . "_label"} }}</td>
            ...
            <tr class="text-gray-700">
                <td class="border px-4 py-2">
                    6. Form K
                    @if($formKState >= 1)
                        <i class="fas fa-file-pdf" wire:click="exportPDF('{{$formKUuid}}', 'livewire.partials.form-k', '6. K	Establishing of the controlling person of operating legal entities')" style="cursor: pointer; float: right;"></i>
                    @endif
                </td>
                <td class="border px-4 py-2">
                    @if($formKState >= 2 && in_array(auth()->user()->type, [0]))
                        Accepted
                    @else
                        <a href="{{ route('form-k', ['customerUuid' => $customerUuid]) }}">Go to form</a>
                    @endif
                </td>
                <td class="border px-4 py-2"></td>
                <td class="border px-4 py-2 justify-center">
            ...
            @for ($i = 0 ; $i < 3; $i++)
                ...
                <tr class="text-gray-700">
                    ...
                    <td class="border px-4 py-2">
                        3. KYC form - individuals
                        @if(${"kycFormState_$i"} >= 1)
                            <i class="fas fa-file-pdf" wire:click="exportPDF('{{$kycIds[$i]}}', 'livewire.partials.partial-k-y-c-form-individual', '3. KYC form - individuals')" style="cursor: pointer; float: right;"></i>
                        @endif
                    </td>
                    <td class="border px-4 py-2">
                        @if(${"kycFormState_$i"} >= 2 && in_array(auth()->user()->type, [0]))
                            Accepted
                        @else
                            <a href="{{ route('kyc-form-individual', ['customerUuid' => $customerUuid, 'formId' => $kycIds[$i]]) }}">Go to form</a>
                        @endif
                    </td>
                    <td class="border px-4 py-2"></td>
                    <td class="border px-4 py-2 justify-center">
                    ...
                    <td class="border px-4 py-2 justify-center">
                        @if(${"kycFormState_$i"} >= 2)
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6 text-green-500">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                        @elseif(${"kycFormState_$i"} == -1)
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6 text-red-500">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        @endif
                ...
                <tr class="text-gray-700">
                    <td class="border px-4 py-2">
                        5. Form A
                        @if(${"formAState_$i"} >= 1)
                            <i class="fas fa-file-pdf" wire:click="exportPDF('{{$aIds[$i]}}', 'livewire.partials.form-a', '5. A	Declaration of identity of the beneficial owner')" style="cursor: pointer; float: right;"></i>
                        @endif
                    </td>
                    <td class="border px-4 py-2">
                        @if(${"formAState_$i"} >= 2 && in_array(auth()->user()->type, [0]))
                            Accepted
                        @else
                            <a href="{{ route('form-a', ['customerUuid' => $customerUuid, 'formId' => $aIds[$i]]) }}">Go to form</a>
                        @endif
                    </td>
                    <td class="border px-4 py-2"></td>
                    <td class="border px-4 py-2 justify-center">
                    ...
                    <td class="border px-4 py-2 justify-center">
                        ...
                        @elseif(${"formAState_$i"} == -1)
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6 text-red-500">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        @endif
                        {{ ${"formARejectReason_$i"} }}
                    </td>
                    <td class="border px-4 py-2"></td>
                </tr>
            @endfor
